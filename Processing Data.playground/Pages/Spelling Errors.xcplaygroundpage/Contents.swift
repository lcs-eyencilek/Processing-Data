...
for data in surveyData {
    if showCatalog.contains(data.lowercased()) {
        tabulator.incrementCount(forValue: data)
    } else {
        tabulator.incrementCount(forValue: closestMatch(for: data, from: tabulator.values))
    }
}
...
for value in tabulator.values {
    if showCatalog.contains(value.lowercased()) {
        print(value)
    }
}
// Print a header
