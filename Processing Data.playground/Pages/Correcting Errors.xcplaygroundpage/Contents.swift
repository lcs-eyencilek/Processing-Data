...
var catalogLowercased: [String] = []
// For all shows in showCatalog, add a lowercase version to the array.
for value in showCatalog {
    catalogLowercased.append(value.lowercased())
}
...
for data in surveyData {
    catalogLowercased.append(data.lowercased())
...
for value in tabulator.values {
    if catalogLowercased.contains(value.lowercased()) {
        print(value)
    }
}
// Print a header
