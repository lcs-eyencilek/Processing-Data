...
func indexOfMaximum(from tabulatedValues: [TabulatedValue]) -> Int {
    ...
    for i in 0...tabulatedValues.count - 1 {
        ...
        let value = tabulatedValues[i]
        let maximumValue = tabulatedValues[maximumIndex]
        
        if value.count > maximumValue.count {
            maximumIndex = i
        }
    }
    return maximumIndex
...
func indexOfMinimum(from tabulatedValues: [TabulatedValue]) -> Int {
    ...
    for i in 0...tabulatedValues.count - 1 {
        ...
        let value = tabulatedValues[i]
        let minimumValue = tabulatedValues[minimumIndex]
        
        if value.count < minimumValue.count {
            minimumIndex = i
        }
    }
    return minimumIndex
...
for i in 0...2 {
    let mostPopularIndex = indexOfMaximum(from: allShows)
    mostPopular.append(allShows[mostPopularIndex])
    allShows.remove(at: mostPopularIndex)
}
...
//  - Remove the show at that index from the all shows array
for i in 0...2 {
    let leastPopularIndex = indexOfMinimum(from: allShows)
    leastPopular.append(allShows[leastPopularIndex])
    allShows.remove(at: leastPopularIndex)
}
